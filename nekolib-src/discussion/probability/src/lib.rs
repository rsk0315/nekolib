...
//! 各遷移は独立に行われるものとする。
//!
//! <span style="font-size: 75%">$`p_{i, j}\gt 0`$ のときに頂点 $`q_i`$ から $`q_j`$ に重み $`p_{i, j}`$ の有向辺が張られるようなグラフをイメージするとよい。</span>
//!
...
//! <span style="font-size: 75%">無限和の入れ替えについては？</span>
//!
...
//! ### Re: Markov chain
//!
//! $`j \lt i \implies p_{i, j} = 0`$ かつ $`i\lt n-1\implies p_{i, i} \in[0\lldot 1)`$
//! で、状態 $`q_0`$ から開始して $`q_{n-1}`$ に到達する時刻の期待値について再度考える。
//!
//! 時刻 $`0`$ に状態 $`q_i`$ から開始して、状態 $`q_i`$ 以外に到達する時刻を $`T_i`$ とおく。
//! $`\P(T_i\ge t) = p_{i, i}^{t-1}`$ であるから、先の補題より
//! ```math
//! \begin{aligned}
//! \E[T_i] &= \sum_{t=1}^{\infty} \P(T_i\ge t) \\
//! &= \sum_{t=1}^{\infty} p_{i, i}^{t-1} \\
// &= \sum_{t=0}^{\infty} p_{i, i}^t \\
//! &= \frac1{1-p_{i, i}}
//! \end{aligned}
//! ```
//! が成り立つ。
//!
//! 各 $`0\le i\lt n-1`$ に対して、状態 $`q_i`$ から遷移する回数を表す確率変数を $`X_i`$
//! とする。すなわち、時刻 $`t`$ に $`q_i`$ にいるような $`t`$ 全体からなる集合の要素数が
//! $`X_i`$ となる。このとき、$`q_{n-1}`$ に到達する時刻を $`X`$ とおくと
//! $`X = \sum_{i=0}^{n-2} X_i`$ が成り立つ。
//! 時刻 $`t`$ に、初めて $`q_i`$ に遷移したとき $`Y_{t, i} = 1`$、そうでないとき
//! $`Y_{t, i} = 0`$ となる確率変数 $`Y_{t, i}`$ を考えると、
//! ```math
//! \begin{aligned}
//! \E[X] &= \E{\left[\sum_{i=0}^{n-2} X_i\right]} \\
//! &= \sum_{i=0}^{n-2} \E[X_i] \\
//! &= \sum_{i=0}^{n-2} \sum_{t=0}^{\infty} \P(Y_{t, i} = 1) \cdot \E[T_i] \\
//! &= \sum_{i=0}^{n-2} \frac1{1-p_{i, i}} \cdot \P{\left(\ExistsL{t\ge 0}{Y_{t, i} = 1}\right)}
//! \end{aligned}
//! ```
//! となる。すなわち、各 $`i`$ に対して状態 $`q_i`$ に一度でも遷移する確率を求める問題に帰着できる。局所的に $`p_{i, j}`$ を変えた場合の $`\E[X]`$ を答えられる状況もある。See also [ABC 144 F](https://atcoder.jp/contests/abc144/tasks/abc144_f).
//!
//! <span style="font-size: 75%">$`j\lt i`$ で $`p_{i, j} = 0`$ が成り立たないときは、状態 $`q_i`$ 自身からの遷移を除いて時刻 $`t`$ に $`q_i`$ に到達するとき $`1`$ となる確率変数を考える必要があるため、$`\P(\ExistsL{t}{Y_{t, i}=1})`$ と置き換えることができなくなる。これを求めるにあたって掃き出し法などが必要になりそう。</span>
//!
//! ## References
//!
