...
impl<I: SbInt, F: Fn(I, I) -> bool> Iterator for FracApproxIter<I, F> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mediant = *lower + *upper;

        if mediant.is_deeper(*bound) {
            return None;
        }
        ...
        while lo.lt1(hi) {
            let mid = lo.avg(hi);
            let tmp = from + to * mid;
            let cur = !tmp.is_deeper(*bound) && pred(tmp) == tf;
...
impl<I: SbInt, F: Fn(I, I) -> bool> FracApprox<I, F> for F {
    ...
    fn frac_approx_bound(self, bound: I) -> [(I, I); 2] {
        let mut lo = I::lowest_frac();
        let mut hi = I::highest_frac();
        for x in FracApproxIter::new(self, Some(bound)) {
            match x {
                (Lower(f), _) => lo = f,
                (Upper(f), _) => hi = f,
            }
...
fn sanity_check() {
    ...
    eprintln!("{:?}", sqrt2.frac_approx_bound(10000));
    ...
    const PI_INT: u128 = 3;
    const PI_FRAC: &str = "141592653589793238462643383279502884197169399375105820974944592307816406286208998628034825342117067982148086513282306647";
    let pi = |p: u128, q| {
        // p/q <= A/B
        if p / q != PI_INT {
            return p / q < PI_INT;
        ...
        let mut r = p % q * 10;
        for d in PI_FRAC.bytes().map(|b| (b - b'0') as u128) {
            if r / q != d {
                return r / q < d;
            }
            r = r % q * 10;
    ...
    for ap in pi.frac_approx_iter_bound(10_u128.pow(18)) {
        eprintln!("{ap:?}");
    }
}
