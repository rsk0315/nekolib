...
impl<'a, T> MutNodeRef<'a, T> {
    fn adjoin(self, sep: T, mut right: Self) -> OwnedNodeRef<T> {
        ...
        let (node, height) = if left.height < right.height {
            ...
            let (node, height) = parent.insert(0, sep, [left, right]);
            if left.is_underfull() {
                left.resolve_underfull_to_root();
            } else {
                (node, height)
            }
            ...
            let (node, height) = parent.insert(init_len, sep, [left, right]);
            if right.is_underfull() {
                right.resolve_underfull_to_root();
            } else {
                (node, height)
            }
        } else {
            ...
            let (node, height) = (InternalNode::as_leaf_ptr(root), height + 1);
            if left.is_underfull() && right.is_underfull() {
                left.merge(right)
            } else if left.is_underfull() || right.is_underfull() {
                left.share(&mut right);
                //
            } else {
                (node, height)
            }
        ...
        // We have to take care of the invariant of `.buflen` of
